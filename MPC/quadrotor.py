...
class Quadrotor(object):
  ...
  def add_input_saturation_constraint(self, prog, x, u, N):
    ...
    for i in range(N-1):
        ...
        for j in range(2):
            u1 = u[i,j]
            prog.AddBoundingBoxConstraint(self.umin-ud[j],self.umax-ud[j],u1)
    pass
  ...
  def add_dynamics_constraint(self, prog, x, u, N, T):
    ...
    value =np.zeros(6) 
    for i in range(N-1):
        exp = x[i+1] -A@x[i] -B@(u[i])#-self.u_d())
        prog.AddLinearEqualityConstraint(exp,value)
  ...
  def add_cost(self, prog, x, u, N):
    ...
    ud = self.u_d()
    for i in range(N-1):
      prog.AddQuadraticCost((x[i]).T@self.Q@(x[i]))# + (u[i]-self.u_d()).T@self.R @(u[i]-self.u_d()))
  ...
  def compute_mpc_feedback(self, x_current, use_clf=False):
    ...
    u_mpc = self.u_d() +result.GetSolution(u[0])
