...
from pos_constraints import Ab_i2
from pos_constraints import Ab_i3
from pos_constraints import Ab_i4
from pos_constraints import Ab_i5
...
def add_continuity_constraints(prog, sigma, n, d, dt):
  ...
  for i in range(n-1):
    Aeq_i, beq_i = Ab_i2(i, n, d, dt[i])
    prog.AddLinearEqualityConstraint(Aeq_i, beq_i, sigma.flatten())
    Aeq_i, beq_i = Ab_i3(i, n, d, dt[i])
    prog.AddLinearEqualityConstraint(Aeq_i, beq_i, sigma.flatten())
    Aeq_i, beq_i = Ab_i4(i, n, d, dt[i])
    prog.AddLinearEqualityConstraint(Aeq_i, beq_i, sigma.flatten())
    Aeq_i, beq_i = Ab_i5(i, n, d, dt[i])
    prog.AddLinearEqualityConstraint(Aeq_i, beq_i, sigma.flatten())
...
def add_minsnap_cost(prog, sigma, n, d, dt):
  ...
  sigmay = np.zeros((d*n,1), dtype="object")
  sigmaz = np.zeros((d*n,1), dtype="object")
  b= np.zeros((d*n,1))
  for ii in range(n*d):
      sigmay[ii] = sigma.flatten()[2*ii]
      sigmaz[ii] = sigma.flatten()[2*ii+1]

  for nn in range(n):
      cost1 = np.zeros((d*n,d*n))
      for i in range(4,d):
          for j in range(4,d):
              cost1[nn*d+i,nn*d+j] = 2*i*(i-1)*(i-2)*(i-3)*j*(j-1)*(j-2)*(j-3)*(dt[nn]**(i+j-7))/(i+j-7)
      prog.AddQuadraticCost(cost1,b, sigmay)
      prog.AddQuadraticCost(cost1,b, sigmaz)
